...
public:
    ...
    static bool HandleAnticheatWarnCommand(ChatHandler* handler, const char* args)
    {
        ...
        char* buf = strdup("Das Anticheat System hat mehrfach berichtet das sie Cheaten. Bitte lassen sie es nun sein oder sie werden vom Server verbannt.");
        char* pos = buf;
    ...
    static bool HandleAntiCheatHandleCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (strCommand.compare("on") == 0)
        {
            ...
            handler->SendSysMessage("Anticheat System wurder erfolgreicht aktiviert");
        }
        else if (strCommand.compare("off") == 0)
        {
            sWorld->setBoolConfig(CONFIG_ANTICHEAT_ENABLE,false);
            handler->SendSysMessage("Anticheat System wurder erfolgreicht deaktiviert");
        }
    ...
    static bool HandleAntiCheatGlobalCommand(ChatHandler* handler, const char* args)
    {
        if (!sWorld->getBoolConfig(CONFIG_ANTICHEAT_ENABLE))
        {
            handler->PSendSysMessage("Anticheat system ist deaktiviert.");
            return true;
        }
