...
public:
    ...
    struct boss_sartharionAI : public BossAI
    {
        ...
        void CastLavaStrikeOnTarget(Unit* target,bool summon)
        {
            if(guidFireCyclone == 0)
            {
                ...
                guidFireCyclone = (*itr)->GetGUID();
            }
            
            if(Creature* LavaCyclon = Creature::GetCreature((*me),guidFireCyclone))
            {
                if(summon)
        ...
        void UpdateAI(const uint32 uiDiff)
        {
            ...
            if (!bBerserk && HealthBelowPct(35) && ((pTene && pTene->isAlive()) || (pShad && pShad->isAlive()) || (pVesp && pVesp->isAlive())))
            {
            ...
            if (!bSoftEnraged && HealthBelowPct(10))
                bSoftEnraged = true;

            if (LavaStrikeTimer <= uiDiff)
            {
                if(bFireCycloneFireing)
                {
                if (Unit* pTarget = SelectTarget(SELECT_TARGET_RANDOM, 0))
                    ...
                    }else
                    {
                        ...
                        bFireCycloneFireing = false;
                        guidFireCyclone = 0;
                        LavaStrikeTimer = (bSoftEnraged ? urand(1400, 2000) : urand(20000,25000));
                    }
                    FireCount++;
                }
                else
    ...
    private:
        ...
        bool bBerserk;
        bool bSoftEnraged;
        bool bFireCycloneFireing;

        uint64 guidFireCyclone;
        uint32 FireCount;
...
#define bShield_ON_SHADRON                      30
#define bShield_ON_SARTHARION                   40
...
struct dummy_dragonAI : public ScriptedAI
{
    ...
    void OpenPortal()
    {
        ...
        if (pPortal && !pPortal->isSpawned())
            pPortal->SetRespawnTime(PortalRespawnTime);
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        if (bCanMoveFree && MoveNextTimer)
        {
            if (MoveNextTimer <= uiDiff)
            {
                if (WaypointId < MAX_WAYPOINT)
                    me->GetMotionMaster()->MovePoint(WaypointId, m_aDragonCommon[WaypointId]);
                
                MoveNextTimer = 0;
            }
            else MoveNextTimer -= uiDiff;
        }
    }
};
...
public:
    ...
    struct mob_tenebronAI : public dummy_dragonAI
    {
        ...
        void UpdateAI(const uint32 uiDiff)
        {
            ...
            else ShadowFissureTimer -= uiDiff;

            if (HatchEggTimer <= uiDiff)
            {
                if(bHasPortalOpen)
                    HatchEggTimer = 10000;
                else
                {
                    OpenPortal();
                    HatchEggTimer = urand(40000,50000);
                }
            }
            else HatchEggTimer -= uiDiff;

            if (ShadowBreathTimer <= uiDiff)
            {
...
public:
    ...
    struct mob_shadronAI : public dummy_dragonAI
    {
        ...
        void UpdateAI(const uint32 uiDiff)
        {
            ...
            else AcolyteShadronTimer -= uiDiff;
...
public:
    ...
    struct mob_vesperonAI : public dummy_dragonAI
    {
        ...
        void UpdateAI(const uint32 uiDiff)
        {
            ...
            if (AcolyteVesperonTimer <= uiDiff)
            {
                if(bHasPortalOpen)
                    return;
                else
            ...
            else AcolyteVesperonTimer -= uiDiff;
...
public:
    ...
    struct mob_acolyte_of_shadronAI : public ScriptedAI
    {
        ...
        void JustDied(Unit* killer)
        {
            ...
            else
            {
                if (pInstance)
                {
                    ...
                    if(Shadron)
                        (CAST_AI(mob_shadron::mob_shadronAI,Shadron->AI()))->bHasPortalOpen = false;                
        ...
        void UpdateAI(const uint32 uiDiff)
        {
            if (!bShield && ShieldTimer <= uiDiff)
            {
                Creature* pShadron = Unit::GetCreature(*me, pInstance->GetData64(DATA_SHADRON));
                if (pInstance->GetData(TYPE_SARTHARION_EVENT) == IN_PROGRESS)
                    pShadron->AI()->DoAction(bShield_ON_SARTHARION);
                else            
                    pShadron->AI()->DoAction(bShield_ON_SHADRON);
    ...
    struct mob_acolyte_of_vesperonAI : public ScriptedAI
    {
        ...
        void JustDied(Unit* pKiller)
        {
            if (pInstance)
            {
                ...
                if (pVesperon)
                    (CAST_AI(mob_vesperon::mob_vesperonAI,pVesperon->AI()))->bHasPortalOpen = false;
        ...
        void UpdateAI(const uint32 uiDiff)
        {
            if (bToInterrupt && CheckForInterrupt <= uiDiff)
            {
    ...
    struct mob_twilight_eggsAI : public Scripted_NoMovementAI
    {
        ...
        void UpdateAI(const uint32 uiDiff)
        {
            if (HatchEggTimer <= uiDiff)
            {
                ...
                if (Tenebron) 
                   (CAST_AI(mob_tenebron::mob_tenebronAI,Tenebron->AI()))->bHasPortalOpen = false;
                SpawnWhelps();
                HatchEggTimer = urand(40000,50000);
            }
            else HatchEggTimer -= uiDiff;
