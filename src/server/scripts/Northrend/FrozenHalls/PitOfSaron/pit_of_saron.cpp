...
class mob_icicle_summon : public CreatureScript
{
public:
    mob_icicle_summon() : CreatureScript("mob_icicle_summon") { }

    CreatureAI* GetAI(Creature* creature) const
    {
        return new mob_icicle_summonAI (creature);
    }

    struct mob_icicle_summonAI : public Scripted_NoMovementAI
    {
        mob_icicle_summonAI(Creature *creature) : Scripted_NoMovementAI(creature)
        {
            instance = (InstanceScript *) creature->GetInstanceScript();
        }

        uint32 uiPlayerCheckTimer;
        uint32 uiPlayerCheckTimer2;
        ...
        void Reset()
        {
            ...
            uiPlayerCheckTimer = 3000;
            uiPlayerCheckTimer2 = 6000;
        }
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            if (uiPlayerCheckTimer < diff)
            {
                if (me->SelectNearestTarget(10.0f))
                if (Unit *pTarget = SelectTarget(SELECT_TARGET_RANDOM, 0, 15, true))
                {
                    if (pTarget->isAlive())
                        pTarget->CastSpell(pTarget, SPELL_ICICLE, true);
                }
                uiPlayerCheckTimer = urand(7000, 10000);
            }
            else uiPlayerCheckTimer -= diff;

            if (uiPlayerCheckTimer2 < diff)
            {
                me->Kill(me);
                me->Respawn(true);
                uiPlayerCheckTimer2 = urand(2000,3000);
            }
            else uiPlayerCheckTimer2 -= diff;
        }
    };
};
