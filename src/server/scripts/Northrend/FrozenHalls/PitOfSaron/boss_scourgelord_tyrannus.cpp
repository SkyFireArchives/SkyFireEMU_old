...
struct outrotyrannusPosition
{
    uint32 entry[2];
    Position movePosition;
} outrotyrannusPosition[] =
{
    { { NPC_JAINA_PART2, NPC_SYLVANAS_PART2 }, { 1018.376f, 167.2495f, 628.2811f, 0.000000f } },
...
class boss_tyrannus : public CreatureScript
{
    public:
        ...
        struct boss_tyrannusAI : public BossAI
        {
            ...
            {
                Position spawnPoint = {1018.376f, 167.2495f, 628.2811f, 0.000000f};
                ...
                for (int8 i = 0; outrotyrannusPosition[i].entry[entryIndex] != 0; ++i)
                {
                    if (Creature *summon = me->SummonCreature(outrotyrannusPosition[i].entry[entryIndex], spawnPoint, TEMPSUMMON_DEAD_DESPAWN))
                    {
                        summon->GetMotionMaster()->MovePoint(0, outrotyrannusPosition[i].movePosition);

                        if (summon->GetEntry() == NPC_JAINA_PART2)
                            DoScriptText(SAY_JAYNA_OUTRO_4, summon);
                        else if (summon->GetEntry() == NPC_SYLVANAS_PART2)
                            DoScriptText(SAY_SYLVANAS_OUTRO_4, summon);
                    }
                }
            }
            ...
            void UpdateAI(const uint32 diff)
            {
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    ...
                    {
                        case EVENT_INTRO_1:
                            //if (Creature* GorkunMartin = me->GetCreature(*me, instance->GetData64(DATA_GORKUN_MARTIN_1)))
                            //    DoScriptText(SAY_GORKUN_INTRO_2, GorkunMartin);
                            break;
                        case EVENT_INTRO_2:
        ...
        struct boss_rimefangAI : public ScriptedAI
        {
            ...
            void DoAction(const int32 actionId)
            {
                if (actionId == ACTION_START_RIMEFANG)
                {
                    ...
                    _events.ScheduleEvent(EVENT_ICY_BLAST, 6000, 0, PHASE_COMBAT);
                }
...
class spell_rimefang_pos_icy_blast : public SpellScriptLoader
{
public:
    spell_rimefang_pos_icy_blast() : SpellScriptLoader("spell_rimefang_pos_icy_blast") { }

    class spell_rimefang_icy_pos_blast_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_rimefang_icy_pos_blast_SpellScript);

        bool Validate(SpellEntry const* /*spell*/)
        {
            if (!sSpellStore.LookupEntry(SPELL_ICY_BLAST_AURA))
                return false;
            return true;
        }
        ...
        void Register()
        {
            OnEffect += SpellEffectFn(spell_rimefang_icy_pos_blast_SpellScript::HandleTriggerMissile, EFFECT_1, SPELL_EFFECT_TRIGGER_MISSILE);
        }
    };

    SpellScript* GetSpellScript() const
    {
        return new spell_rimefang_icy_pos_blast_SpellScript();
    }
};
...
void AddSC_boss_tyrannus()
{
    ...
    new spell_rimefang_pos_icy_blast();
}
