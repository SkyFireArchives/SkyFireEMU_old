            ...
            while(uint32 eventId = events.ExecuteEvent())
            {
                switch(eventId)
                {
                    ...
                    case EVENT_GRAB_POT:
                        ...
                        {
                            SlagPotTarget->EnterVehicle(vehicle, 0);
                    ...
                    case EVENT_CONSTRUCT:
                        if (!construct_list.empty())
                        {
                            ...
                            {
                                if (Is25ManRaid())
                                    me->CastCustomSpell(SPELL_ACTIVATE_CONSTRUCT, SPELLVALUE_MAX_TARGETS, 2, pTarget, false);
                                else
                                    me->CastCustomSpell(SPELL_ACTIVATE_CONSTRUCT, SPELLVALUE_MAX_TARGETS, 1, pTarget, false);
