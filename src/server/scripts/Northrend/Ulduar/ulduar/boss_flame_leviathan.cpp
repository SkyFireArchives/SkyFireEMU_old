                    ...
                    if (vehicle->GetPassenger(i))
                        if (Vehicle *pSeat = vehicle->GetPassenger(i)->GetVehicleKit())
                        {
                            if (Creature* pTurret = (me->SummonCreature(NPC_TURRET, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 0, TEMPSUMMON_MANUAL_DESPAWN)))
                                pTurret->EnterVehicle(vehicle, SEAT_TURRET);
                            ...
                            if (Creature* pDevice = (me->SummonCreature(NPC_DEVICE, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 0, TEMPSUMMON_MANUAL_DESPAWN)))
                                pDevice->EnterVehicle(vehicle, SEAT_DEVICE);
                        }
                }
            }
            else
...
#define GOSSIP_ITEM_3  "Deactivated the Tower of Storms"
#define GOSSIP_ITEM_4  "Deactivated the Tower of Flames"
#define GOSSIP_ITEM_5  "Deactivated the Tower of Frost"
#define GOSSIP_ITEM_6  "Deactivated the Tower of Life"
#define GOSSIP_ITEM_7  "Deactivated secondary defensive systems"
    ...
    bool OnGossipSelect(Player* pPlayer, Creature* pCreature, uint32 /*uiSender*/, uint32 uiAction)
    {
        ...
        switch(uiAction)
        {
            ...
            case GOSSIP_ACTION_INFO_DEF+3:
                pPlayer->CLOSE_GOSSIP_MENU();
                if (Creature* pLeviathan = instance->instance->GetCreature(instance->GetData64(DATA_LEVIATHAN)))
                    pLeviathan->AI()->DoAction(ACTION_TOWER_OF_STORMS); 
                break;
            case GOSSIP_ACTION_INFO_DEF+4:
                pPlayer->CLOSE_GOSSIP_MENU();
                if (Creature* pLeviathan = instance->instance->GetCreature(instance->GetData64(DATA_LEVIATHAN)))
                     pLeviathan->AI()->DoAction(ACTION_TOWER_OF_FLAMES);
                break;
            case GOSSIP_ACTION_INFO_DEF+5:
                pPlayer->CLOSE_GOSSIP_MENU();
                if (Creature* pLeviathan = instance->instance->GetCreature(instance->GetData64(DATA_LEVIATHAN)))
                     pLeviathan->AI()->DoAction(ACTION_TOWER_OF_FROST);
                break;
            case GOSSIP_ACTION_INFO_DEF+6:
                pPlayer->CLOSE_GOSSIP_MENU();
                if (Creature* pLeviathan = instance->instance->GetCreature(instance->GetData64(DATA_LEVIATHAN)))
                     pLeviathan->AI()->DoAction(ACTION_TOWER_OF_LIFE);
                break;
