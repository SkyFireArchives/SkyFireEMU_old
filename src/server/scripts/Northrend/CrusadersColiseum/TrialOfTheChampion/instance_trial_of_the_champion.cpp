...
enum eEnums
{
    SAY_GRAND_DONE                            = -1649519,
    SAY_START_2                               = -1649529,
    SAY_E_P_DONE                              = -1649546,
};
    ...
    struct instance_trial_of_the_champion_InstanceMapScript : public InstanceScript
    {
        ...
        void OnCreatureCreate(Creature *pCreature, bool /*bAdd*/)
        {
            ...
            case NPC_BLACK_KNIGHT:
                ...
                break;
            case NPC_ARMORY_OF_GHOUL:
                uiChampionGhoulGUID = pCreature->GetGUID();
                break;
            case NPC_TIRION:
                uiTirionGUID = pCreature->GetGUID();
                break;
            }
        }
            ...
            switch(type)
            {
                ...
                case BOSS_GRAND_CHAMPIONS:
                    ...
                    else if (data == DONE)
                    {
                        ...
                        {		
                            if (Creature* pTirion =  instance->GetCreature(uiTirionGUID))
                                if (Creature* pAnnouncer =  instance->GetCreature(uiAnnouncerGUID))
                            {
                                DoScriptText(SAY_GRAND_DONE, pTirion);
                ...
                case DATA_ARGENT_SOLDIER_DEFEATED:
                    ...
                    if (uiArgentSoldierDeaths == 9)
                    {
                        ...
                        if (Creature* pTirion = instance->GetCreature(uiTirionGUID))
                        {
                            DoScriptText(SAY_START_2, pTirion);
                        }
                    }
                    break;
                case BOSS_ARGENT_CHALLENGE_E:
                    ...
                    if (Creature* pTirion = instance->GetCreature(uiTirionGUID))
                        if (Creature* pAnnouncer =  instance->GetCreature(uiAnnouncerGUID))
                    {
                        DoScriptText(SAY_E_P_DONE, pTirion);
                        pAnnouncer->SetFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_GOSSIP);
        ...
        uint64 GetData64(uint32 data)
        {
            ...
            case DATA_BLACK_KNIGHT:
                return uiBlackKnightGUID;
            case DATA_CHAMPION_GHOUL:
                return uiChampionGhoulGUID;
            case DATA_TIRION:
                return uiTirionGUID;
            }
            return 0;
        ...
        void SetData64(uint32 uiType, uint64 data)
        {
            switch(uiType)
            {
            case DATA_GRAND_CHAMPION_1:
                uiGrandChampion1GUID = data;
                break;
            case DATA_GRAND_CHAMPION_2:
                uiGrandChampion2GUID = data;
                break;
            case DATA_GRAND_CHAMPION_3:
                uiGrandChampion3GUID = data;
                break;
            }
        }
